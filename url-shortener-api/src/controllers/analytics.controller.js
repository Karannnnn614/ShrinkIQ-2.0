...
export const getUserAnalytics = async (req, res) => {
    try {
        ...
        const analyticsData = await Promise.all(links.map(async ({ originalUrl, shortUrl, createdAt, expirationDate }) => {
...
export const getClicksOverTime = async (req, res) => {
    try {
        ...
        const chartData = await Promise.all(links.map(async ({ shortUrl }) => {
            const clicks = await Click.find({ shortUrl });
